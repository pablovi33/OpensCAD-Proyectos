...
module cajaSCTapa (largo=40,ancho=30,alto=40,espesor=3,dientes=6,modo=1,orgX=2,orgY=2){
...
//base
  ...
  //######ORGANIZADOR X #########
    ...
    //square([largo,ancho],center=true);
    base(largo,ancho,espesor,dientes,0,1,1,1);
        }
...
//lados alto
 ...
 difference(){
  ...
  translate([-alto/2-espesor,0])
  if(orgX!=0){
      tamD=(alto)/dientes;
   for(i=[1:(dientes/2)]){
     translate([tamD*2*i,0])
     square([tamD,espesor],center=true);
    }
    
    translate([+alto+(espesor*2.5),0])
  if(orgX!=0){
      tamD=(alto)/dientes;
       translate([tamD,0])
   for(i=[1:(dientes/2)-1]){
...
cajaSCTapa(largo=100,ancho=50,alto=40,espesor=3,dientes=6,modo=1);
